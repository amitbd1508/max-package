...
export class BranchAndBoundStrategy implements Strategy{
    execute(maxCapacity: number, packageItems: PackageItem[]): PackageItem[] {
        ...
        packageItems.sort((x: PackageItem, y: PackageItem) => y.pricePerWeight - x.pricePerWeight);
        ...
        while(priorityQueue.length !== 0) {
            ...
            if(node.bound > maxProfit) {
                ...
                const leftNode = new TreeNode(currentLevel, currentWeight , currentProfit);
                ...
                leftNode.bound = this.getUpperBound(leftNode, maxCapacity, packageItems);

                if(leftNode.bound > maxProfit) {
                    priorityQueue.push(leftNode);
                }

                const rightNode = new TreeNode(leftNode.level, node.weight, node.profit);
                rightNode.bound = this.getUpperBound(rightNode, maxCapacity, packageItems);
    ...
    private getUpperBound(node: TreeNode, capacity: number, packageItems: PackageItem[]) {
        const totalPackage = packageItems.length;
        ...
        while (j < totalPackage && (totalWeight + packageItems[j].weight) <= capacity) {
